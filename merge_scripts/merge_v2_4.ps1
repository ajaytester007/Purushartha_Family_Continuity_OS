...
with tabs[0]:
    ...
    c5.metric("Consent Records", len(consent))
    if not cases.empty:
...
with tabs[5]:
    ...
    consent_df = load_csv(CONSENT_PATH)
    if consent_df.empty:
        st.error("Consent ledger is missing. Report generation is allowed only for synthetic/demo content.")
    else:
        st.success("Consent ledger detected. Public output must remain synthetic or sanitized.")
